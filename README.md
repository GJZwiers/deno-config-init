...
`deno-init` is a simple command line tool to initialize Deno projects from prompts or templates.
...
```bash
deno install --allow-read --allow-run --allow-write --unstable -n deno-init https://deno.land/x/init@0.15.6/mod.ts
...
```bash
deno install --allow-read --allow-run --allow-write --unstable -n deno-init https://x.nest.land/init@0.15.6/mod.ts
```
