...
```
deno install --allow-read --allow-run=git --allow-write --name deno-init https://deno.land/x/init@v.1.5.0/mod.ts
...
```
deno run --allow-read --allow-run=git --allow-write https://raw.githubusercontent.com/GJZwiers/deno-init/v.1.5.0/mod.ts -y -n awesome_deno_project
```
...
```
deno install --allow-read --allow-run=git --allow-write -n deno-init https://deno.land/x/init@v.1.5.0/mod.ts
```
